...
class SystemInfo:
    ...
    def memoryinfo(self):
        meminfo = subprocess.check_output(
            ['bash', '/home/admin/scout-server/app/bashscript/memory/sms_memory_info.sh']).decode(
            'utf-8')
        return meminfo
    ...
    def graphicinfo(self):
        grphinfo = subprocess.check_output(
            ['bash', '/home/admin/scout-server/app/bashscript/graphic/sms_graphic_info.sh']).decode('utf-8')
        return grphinfo

    def cpucoreinfo(self):
        cpuinfo = subprocess.check_output(
            ['bash', '/home/admin/scout-server/app/bashscript/cpu/sms_cpu_info.sh']).decode('utf-8')
        return cpuinfo
